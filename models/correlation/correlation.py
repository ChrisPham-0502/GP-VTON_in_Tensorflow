...
class _FunctionCorrelation(tf.keras.layers.Layer):
    def __init__(self, intStride):
        super(FunctionCorrelation, self).__init__()
        self.intStride = intStride

    def forward(self, first, second):
        rbot0 = tf.zeros([first.shape[0], first.shape[2] + (6 * self.intStride), first.shape[3] + (6 * self.intStride), first.shape[1]])
        rbot1 = tf.zeros([first.shape[0], first.shape[2] + (6 * self.intStride), first.shape[3] + (6 * self.intStride), first.shape[1]])

        output = tf.zeros([first.shape[0], 49, tf.math.ceil(first.shape[2] / self.intStride), tf.math.ceil(first.shape[3] / self.intStride)])
    ...
    def backward(self, gradOutput):
        ...
        gradFirst = tf.zeros([first.shape[0], first.shape[1], first.shape[2], first.shape[3]]) if self.needs_input_grad[0] == True else None
        gradSecond = tf.zeros([first.shape[0], first.shape[1], first.shape[2], first.shape[3]]) if self.needs_input_grad[1] == True else None
...
def FunctionCorrelation(tenFirst, tenSecond, intStride):
    correlation_layer = _FunctionCorrelation(intStride)
    return correlation_layer(tenFirst, tenSecond)

class ModuleCorrelation(tf.keras.layers.Layer):
    def __init__(self):
        super(ModuleCorrelation, self).__init__()

    def call(self, tenFirst, tenSecond, intStride):
        return _FunctionCorrelation(tenFirst, tenSecond, intStride)